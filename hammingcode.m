...
modulationchoice=input('Default modulation is shift keying \n for Energy dispersal and To overcome interfernce\n Enter 1 to choose Spread spectrum ');
...
linecode=input(' Default line coding is URNZ..,\n press 1 for PNRZ \n press 2 for manchester');

if(linecode==1)
     disp('PRNZ-polar non return to zero is chosen');

     pnrz(data);
    
elseif (linecode==2)
        
    disp(' manchester line coding is chosen');

    manchester(data);

else
    
     disp('URNZ-polar non return to zero is chosen');
